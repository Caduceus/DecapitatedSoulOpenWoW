...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    switch (petType)
    {
        ...
        case HUNTER_PET:
        {
			SetCreateHealth(GetOwner()->GetMaxHealth() * 0.8f); //custom
...
{
    uint8 points = (level >= 10) ? (level / 4) : 0;   //uint8 points = (level >= 20) ? ((level - 10) / 4) : 0;
    ...
    switch (getPetType())
    {
        ...
        case HUNTER_PET:
            if (getLevel() > owner->getLevel())
				...
				GivePetLevel(owner->getLevel()); //GivePetLevel(owner->getLevel() - 5); //custom
            break;
        default:
            break;
    }
