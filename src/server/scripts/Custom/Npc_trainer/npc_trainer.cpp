...
void TeachSpells(Creature* creature, Player * plr, uint32 Class)
{
	...
	creature->Whisper("I have tought you all I know of the $C class. The battlefield is a scene of constant chaos. The winner will be the one who controls that chaos, both his own and the enemies. Shall you exceed all of my expectations, $N. Now Go! Make me proud!", LANG_UNIVERSAL, plr);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
	{
		...
		player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR,  "|cff00ff00|TInterface\\icons\\ABILITY_MAGE_WINTERSGRASP:24|t|r I want to reset my talents", GOSSIP_SENDER_MAIN, 101);
	...
	bool OnGossipSelect(Player * player, Creature * creature, uint32 uiSender, uint32 uiAction) 
	{
		...
		switch(uiAction)
		{
			...
			case 100:
			{
				...
				{
					/*ChatHandler(player->GetSession()).PSendSysMessage("You have not reached the required level. Come back when you mature a bit more.");*/
					creature->Whisper("You have not reached the required level. Come back when you mature a bit more.", LANG_UNIVERSAL, player);
					player->PlayerTalkClass->SendCloseGossip();
				}
			}break;
