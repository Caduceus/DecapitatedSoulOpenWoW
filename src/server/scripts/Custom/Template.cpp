...
class some_name : public CreatureScript
{
public:
	some_name() : CreatureScript("some name") { }

	struct some_nameAI : public ScriptedAI
	{
		some_nameAI(Creature* creature) : ScriptedAI(creature) { }
		...
		void EnterCombat(Unit* /*who*/) override
		{
			...
			Talk(SAY_AGGRO);
			events.ScheduleEvent(EVENT_SOME_NAME, 1000);
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				switch (eventId)
				{
					...
					DoCastVictim(SPELL_SOME_NAME);
					if (me->GetVictim() && me->EnsureVictim()->HasAura(SPELL_SOME_NAME))
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 100, true))
							me->TauntApply(target);
					events.ScheduleEvent(EVENT_SOME_NAME, 30000);
					break;
				case EVENT_SOME_NAME:
					DoCastVictim(SPELL_SOME_NAME);
					events.ScheduleEvent(EVENT_SOME_NAME, 10000);
					break;
				default:
					break;
				}
	...
	{
		return new some_nameAI(creature);
	}
};

void AddSC_some_name()
{
	new some_name();
}
